...
var _nesthydrationjs2 = _interopRequireDefault(_nesthydrationjs);
...
const nest = (0, _nesthydrationjs2.default)().nest;
...
function validate(rows) {
  if (Array.isArray(rows)) return rows;else if (rows && rows.rows) return rows.rows;else {
      throw new Error(`"dbCall" function must return/resolve an array of objects where each object is a row from the result set. Instead got ${_util2.default.inspect(rows, { depth: 3 })}`);
    }
}